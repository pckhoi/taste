...
# This script installs taste on your Linux or macOS computer.
...
VERSION=%%version%%
RELEASES_BASE_URL=https://github.com/pckhoi/taste/releases/download/"$VERSION"
INSTALL_URL=$RELEASES_BASE_URL/install.sh

CURL_USER_AGENT=${CURL_USER_AGENT:-taste-installer}
...
assert_linux_or_macos() {
  ...
  if [ "$OS" == Linux ]; then
    ...
    fail "E_UNSUPPORTED_OS" "taste install.sh only supports macOS and Linux."
  fi
  ...
  if [ "$ARCH" == x86_64 ]; then
    ...
    fail "E_UNSUPPOSED_ARCH" "taste install.sh only supports installing taste on x86_64 or arm64."
  fi
}
...
assert_uid_zero() {
  uid=`id -u`
  if [ "$uid" != 0 ]; then
    fail "E_UID_NONZERO" "taste install.sh must run as root; please try running with sudo or running\n\`sudo bash -c 'curl -L $INSTALL_URL | bash'\`."
  fi
}

create_workdir() {
  WORK_DIR=`mktemp -d -t taste-installer.XXXXXX`
  cleanup() {
    rm -rf "$WORK_DIR"
  }
  trap cleanup EXIT
...
install_binary_release() {
  local FILE=$PLATFORM_TUPLE.tar.gz
  local URL=$RELEASES_BASE_URL/$FILE
  echo "Downloading:" $URL
  curl -A "$CURL_USER_AGENT" -fsL "$URL" > "$FILE"
  tar zxf "$FILE"
  echo "Installing taste to /usr/local/bin."
  [ -d /usr/local/bin ] || install -o 0 -g 0 -d /usr/local/bin
  install -o 0 -g 0 $PLATFORM_TUPLE/bin/taste /usr/local/bin
  install -o 0 -g 0 -d /usr/local/share/doc/taste/
  install -o 0 -g 0 -m 644 $PLATFORM_TUPLE/LICENSE /usr/local/share/doc/taste/
}
